...
import { ref, onValue, set, get, update, db } from "../config/firebase";
import { Participant } from "../types";
import { Box, Button, Card, CardContent, Typography, Alert, TextField, InputAdornment, IconButton, List, ListItem, ListItemAvatar, ListItemText, Avatar, Chip, CircularProgress, Stack, Paper, Backdrop, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
import { styled, keyframes } from '@mui/material/styles';
import { SportsEsports as SportsEsportsIcon, Person as PersonIcon, Casino as CasinoIcon, Rocket as RocketIcon, Group as GroupIcon, Share as ShareIcon, PlayArrow as PlayArrowIcon, QrCode as QrCodeIcon, Close as CloseIcon } from '@mui/icons-material';
...
const WaitingRoom: React.FC = () => {
  ...
  return (
    <GradientBox>
      ...
      <MainCard>
        <CardContent>
          ...
          <Stack direction="row" spacing={1}>
            <Button variant="contained" onClick={copyRoomLink} startIcon={<ShareIcon />} sx={{ flex: 1 }}>Copy Link</Button>
            <Button variant="outlined" onClick={showQRCode} startIcon={<QrCodeIcon />} sx={{ flex: 1 }}>QR Code</Button>
          </Stack>
        </CardContent>
      </MainCard>
      ...
      {/* QR Code Dialog */}
      <Dialog open={showQRDialog} onClose={closeQRDialog} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ textAlign: 'center', position: 'relative' }}>
          <Typography variant="h6" fontWeight="bold">Quét QR Code để tham gia</Typography>
          <IconButton 
        ...
        <DialogActions sx={{ justifyContent: 'center', pb: 2 }}>
          <Button onClick={copyRoomLink} variant="outlined" startIcon={<ShareIcon />}>
            Copy Link
          </Button>
        </DialogActions>
      </Dialog>
