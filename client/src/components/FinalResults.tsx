...
const FinalResults: React.FC = () => {
  ...
  const [players, setPlayers] = useState<Player[]>([]);
  const [saved, setSaved] = useState(false);
  const [showTop, setShowTop] = useState([false, false, false]);

  // Lấy người chơi từ Firebase
  useEffect(() => {
    const participantsRef = ref(
      db,
      `quizzes/${quizId}/rooms/${roomId}/participants`
    );
    const unsubscribe = onValue(participantsRef, (snapshot) => {
      if (snapshot.exists()) {
        const data = Object.values(snapshot.val()) as Player[];
        ...
        // Hiện top theo thứ tự
        setTimeout(() => {
          setShowTop((prev) => [true, prev[1], prev[2]]);
          confetti({ particleCount: 50, spread: 60, origin: { y: 0.2 } });
        }, 1200);

        setTimeout(() => {
          setShowTop((prev) => [prev[0], true, prev[2]]);
          confetti({ particleCount: 60, spread: 80, origin: { x: 0.3 } });
        }, 2800);

        setTimeout(() => {
          setShowTop((prev) => [prev[0], prev[1], true]);
  ...
  useEffect(() => {
    if (players.length > 0 && !saved) {
      ...
      apiService
        .createFinalResults(
          Number(quizId),
          currentPlayer ? currentPlayer.score : players[0].score,
          currentPlayer ? currentPlayer.displayName : "chưa login",
          token || undefined
        )
        .catch((err) => console.error("Lỗi lưu kết quả:", err));
    }
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          "linear-gradient(135deg, rgba(241, 246, 244, 0), rgba(244, 247, 249, 0.052)), url('/img/10203647.jpg')",
        backgroundSize: "cover",
        backgroundPosition: "center",
        backgroundRepeat: "no-repeat",
      }}
    >
      ...
      <div className={styles.podium}>
        {/* Top 2 */}
        {players.length >= 2 && showTop[1] && top3[1] && (
          <div className={styles.playerSecond}>
            <motion.div variants={podiumVariants} initial="hidden" animate="visible">
              <motion.div {...bounceAnimation} className={styles.avatarWrapper}>
                <img
                  src={
                    top3[1].avatar ||
                    `https://via.placeholder.com/90?text=${top3[1].displayName.charAt(0)}`
                  }
                  alt={top3[1].displayName}
                  className={styles.avatar}
                />
            ...
            <motion.div variants={podiumVariants} initial="hidden" animate="visible">
              <motion.div {...bounceAnimation} className={styles.avatarWrapper}>
                <img
                  src={
                    top3[0].avatar ||
                    `https://via.placeholder.com/110?text=${top3[0].displayName.charAt(0)}`
                  }
                  alt={top3[0].displayName}
                  className={styles.avatar}
                />
        ...
        {/* Top 3 */}
        {players.length >= 3 && showTop[2] && top3[2] && (
          <div className={styles.playerThird}>
            <motion.div variants={podiumVariants} initial="hidden" animate="visible">
              <motion.div {...bounceAnimation} className={styles.avatarWrapper}>
                <img
                  src={
                    top3[2].avatar ||
                    `https://via.placeholder.com/80?text=${top3[2].displayName.charAt(0)}`
                  }
                  alt={top3[2].displayName}
                  className={styles.avatar}
                />
            ...
            (p, i) =>
              showTop[i] && (
                <div key={i} className={styles.rankItem}>
                  ...
                  <img
                    src={p.avatar || `https://via.placeholder.com/40?text=${p.displayName.charAt(0)}`}
                    alt={p.displayName}
                    className={styles.rankAvatar}
                  />
                  <span className={styles.rankName}>{p.displayName}</span>
                  <span className={styles.rankScore}>{p.score} điểm</span>
                </div>
              )
          )}
          {otherPlayers.map((player, index) => (
